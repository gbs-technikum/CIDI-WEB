...
public class SQL {
	...
	private void verbindung() {
		// Verbindung DB cidi
		try {
			System.out.println("Methode Verbindung");
	...
	public String pruefeLogin(String nutzerName, String pw) {
		try {
			...
			if (rst.next()) {
				...
				String NutzerID = rst.getString(1); // zwischenspeichern der ID
													// des Nutzers der fährt
													// ->bessere Lesbarkeit
				...
				if (!(rst.next())) { // Abfrage ob restTemp ein Element
										// beinhaltet
					...
					sqlBefehl = "INSERT into Sitzung (id_benutzer, beginnSitzung) VALUES ("
							+ NutzerID + ", NOW())";
				...
				System.out
						.println("->Nutzername oder Passwort falsch, bitte nochmal versuchen!");
	...
	public void logout() { // ausstempln wird gesetzt und useFlag wieder auf
							// false
		try {
			this.verbindung(); // Verbing erneut aufbauen, da sie beim eintrag
								// geschlossen worden ist
			// Prüfen ob irgendwo ein true gesetzt ist
			pst = connection
					.prepareStatement("SELECT * FROM benutzer WHERE useFlag=true");
			rst = pst.executeQuery();
			if (rst.next()) { // Abfrage ob rst bei Tabelle Bnutzer ein TRUE
								// beinhaltet
				// useFlag freigeben ... auf false setzen
				sqlBefehl = "UPDATE benutzer SET useFlag=false WHERE id="
						+ rst.getString(1);
				pst.executeUpdate(sqlBefehl); // Abfrage absetzen
				...
				System.out
						.println("-> Zonk .. niemand ist angemeldet (SQL-logout-Methode)");
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
